...
## Companion plugins & skills (external)

The harness is small on purpose and composes with excellent open-source tools
instead of reinventing them (workflow step 0: search & reuse). These install
through their own native mechanisms — they are **not** generated by `install.ps1` —
and the harness references them from its role lenses (`65-roles`) and design rules
(`90-design`). `setup.ps1 -Companions` prints the plugin commands and runs the
shell installers.

| Tool | Role in the harness | Install | License |
|---|---|---|---|
| [diagram-design](https://github.com/cathrynlavery/diagram-design) | Architect lens — editorial architecture / flow / sequence / state diagrams as self-contained HTML+SVG; redraws Mermaid and draw.io | `/plugin marketplace add cathrynlavery/diagram-design` | MIT |
| [Understand-Anything](https://github.com/Egonex-AI/Understand-Anything) | Architect lens — turns a codebase into an interactive knowledge graph with architecture tours and diff-impact analysis | `/plugin marketplace add Egonex-AI/Understand-Anything` | MIT |
| [impeccable](https://github.com/pbakaus/impeccable) | Reviewer / design lens — 61 deterministic UI checks plus live browser iteration for reviewing pages and interfaces (`/impeccable typeset\|distill\|polish`, `npx impeccable detect`) | `/plugin marketplace add pbakaus/impeccable` (or `npx impeccable install`) | Apache-2.0 |
| [taste-skill](https://github.com/Leonxlnx/taste-skill) | Design lens — design "taste" that pushes generated frontends past generic defaults; pairs with a UI/UX design skill | `npx skills add https://github.com/Leonxlnx/taste-skill` | MIT |

The plugin-marketplace commands run inside Claude Code; the `npx` commands run in a
shell. Each tool keeps its own license and updates upstream — the harness documents
and wires them, it does not vendor them.
